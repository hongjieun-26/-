...
//깊이우선탐색(DFS)
class SrchAMGraph :public AdjMatGraph { //AM => adjacent matrix
	bool visited[MAX_VTXS]; // 정점의 방문정보를 저장하는 배열
public:
	void reserVisited() {   //모든 정점을 방문하지 않았다고 초기화 설정
	...
	//DFS 함수
	void DFS(int v) {
		visited[v] = true;  //현재 정점을 방문했다고 수정
		printf("%c ", getVertex(v)); // 현재 정점의 이름 출력
		for (int w = 0;w < size;w++)
			if (isLinked(v, w) && visited[w] == false)
				DFS(w);  //연결+방문x =>> 리컬시브로 방문
